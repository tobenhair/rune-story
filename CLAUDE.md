...
- **Zones**: Aethon City (hub, safe), Village Outskirts (Lv.1+), Verdant Forest (Lv.5+), Crystal Caverns (Lv.10+), Ancient Ruins (Lv.15+), The Hollow Rift (final-boss arena — teleport-only, not on the map), The Endless Rift (endgame wave-survival arena — Riftwarden-only, not on the map), and the **Act 2 region** unlocked by finishing q15: Emberfall Wastes (Lv.20+), Frostveil Glacier (Lv.24+), and The Ashen Sanctum (Act 2 finale arena — teleport-only)
...
- **Daily Rift / Achievements / Codex**: the engagement layer (`META`-stored, account-wide). A deterministic daily challenge (`dailyMods`, **Riftwarden** dialog), ~27 shard-paying achievements (`ACHV`/`checkAchv`, incl. Act 2's `malachar`/`saga2`), and a bestiary that fills as creatures are slain (`bestiaryList`/`CODEX_LORE`, **Chronicler Ily**). See Architecture › Engagement.
