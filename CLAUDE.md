...
- **Zones**: Aethon City (hub, safe), Village Outskirts (Lv.1+), Verdant Forest (Lv.5+), Crystal Caverns (Lv.10+), Ancient Ruins (Lv.15+), The Hollow Rift (Lv.20+, final-boss arena — teleport-only, not on the map), and The Endless Rift (endgame wave-survival arena — Riftwarden-only, not on the map)
...
- **Daily Rift / Achievements / Codex**: the engagement layer (`META`-stored, account-wide). A deterministic daily challenge (`dailyMods`, **Riftwarden** dialog), ~24 shard-paying achievements (`ACHV`/`checkAchv`), and a bestiary that fills as creatures are slain (`bestiaryList`/`CODEX_LORE`, **Chronicler Ily**). See Architecture › Engagement.
