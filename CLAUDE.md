...
XP to next level scales as `xpNext *= 1.4` per level up to 20, then **flattens to `*= 1.15`** from level 20 on (`gainXP`) so the Act 2 band (20–30) stays reachable; `applySave` caps an inflated pre-flattening `xpNext` at the canonical value for the save's level. Each level grants +20 HP, +10 MP, and 1 skill point. Act 2 monsters/quests pay XP in the thousands/tens-of-thousands to match.
